...
- **Next:** R2 + `NEXT_PUBLIC_MEDIA_BASE` (ảnh body — cần credential Cloudflare, agent không tự upload); rà 831 URL baseline (`docs/seo-baseline/seo-slug-diff-report.json`); GSC + logo; DNS/cutover. **Compose:** `env_file: .env.local` → `/api/health` có thể báo `db: ok` sau `docker compose up -d --build` (commit `9d7861d`).
