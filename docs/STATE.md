...
- **Next (thứ tự):** (1) **R2:** xác nhận sync đủ object (nếu chưa) — `docs/STAGING-RUNBOOK.md` §4c. (2) **Nội dung:** product còn ~161 slug thiếu `body_html` sau fallback excerpt; post/page xem `docs/audit/TODO-SNAPSHOT-2026-04-24.md`. (3) **Homepage / UI** theo brand (mockup; hiện `src/app/page.tsx` là hero tối giản). (4) **GSC** (sitemap/property). (5) **DNS cutover.** **Đã xong:** import **933** node (477 product); redirect map + slug diff; logo file trong repo; importer Woo **body_html** từ excerpt khi WXR thiếu `content:encoded`; `lint` / `prelaunch:check` / `staging:smoke` / `perf:budget` PASS trên build gần nhất.
